...
export function ModelDetails() {
  ...
  const { modelId } = useParams({
    from: '/_authenticated/pricing/$modelId/',
  })
  ...
  if (isLoading) {
    return (
      <SectionPageLayout>
        <SectionPageLayout.Title>{t('Model details')}</SectionPageLayout.Title>
  ...
  return (
    <SectionPageLayout>
      <SectionPageLayout.Title>{model.model_name}</SectionPageLayout.Title>
      <SectionPageLayout.Content>
        <div className='mx-auto max-w-5xl'>
          <Button
            variant='ghost'
