...
export function ApiKeysMutateDrawer({
  ...
  const form = useForm<ApiKeyFormValues>({
    ...
    defaultValues: getApiKeyFormDefaultValues(
      forceDefaultGroup ? false : defaultUseAutoGroup
    ),
  })
  ...
  useEffect(() => {
    if (forceDefaultGroup) {
      if (selectedGroup !== DEFAULT_GROUP) {
        form.setValue('group', DEFAULT_GROUP)
      }
      return
    }
  ...
  }, [forceDefaultGroup, groups, form, selectedGroup])

  const onSubmit = async (data: ApiKeyFormValues) => {
    setIsSubmitting(true)
    try {
      const basePayload = transformFormDataToPayload(
        forceDefaultGroup ? { ...data, group: DEFAULT_GROUP } : data
      )
  ...
  const title = isUpdate ? t('Update API Key') : t('Create API Key')
  ...
  const formContent = (
    <Form {...form}>
      <form
        ...
        className={
          embedded ? 'flex flex-col gap-5' : sideDrawerFormClassName('gap-5')
        }
      >
        <SideDrawerSection>
          <SideDrawerSectionHeader
          ...
          {!forceDefaultGroup && selectedGroup === 'auto' && (
            <FormField
              control={form.control}
              name='auto_groups'
              render={({ field }) => (
  ...
  if (isSubmitting) {
    ...
    submitLabel = t('Create API Key and continue')
  }
  const submitButton = (
    <Button
      type='button'
  ...
  if (embedded) {
    return (
      <Card>
        ...
        <CardFooter className='justify-end'>{submitButton}</CardFooter>
      </Card>
    )
  }
