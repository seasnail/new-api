...
func TestPlaygroundResponsesPreservesRequestedGroup(t *testing.T) {
	for _, group := range []string{"", "vip"} {
		t.Run(group, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodPost, "/pg/responses", strings.NewReader(string(body)))
...
func TestPlaygroundResponsesRejectsUnauthorizedGroupBeforeChannelSelection(t *testing.T) {
	...
	router := gin.New()
	router.POST("/pg/responses", func(c *gin.Context) {
		common.SetContextKey(c, constant.ContextKeyUsingGroup, "default")
	}, Distribute(), func(c *gin.Context) { t.Error("unauthorized request reached handler") })
	request := httptest.NewRequest(http.MethodPost, "/pg/responses", strings.NewReader(`{"model":"gpt-test","group":"forbidden-playground-group","input":"hello"}`))
